...
function changeColor(imagePath, colorName) {
    document.getElementById("mainImage").src = imagePath;
    var colorOptions = document.getElementsByClassName("color-option");
    for (var i = 0; i < colorOptions.length; i++) {
        colorOptions[i].classList.remove("selected");
    }
    document.querySelector("[onclick=\"changeColor('" + imagePath + "', '" + colorName + "')\"]").classList.add("selected");
    document.getElementById("selectedColor").innerText = colorName; // עדכון צבע נבחר
}

function toggleTabContent(tabId) {
    var tabContent = document.getElementById(tabId);
    var tabButton = document.querySelector("[onclick=\"toggleTabContent('" + tabId + "')\"] .tab-icon");
